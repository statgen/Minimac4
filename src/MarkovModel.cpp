...
void MarkovModel::ImputeSitesMinimac3(int hapID,int group)
{
    ...
    PrevRightFoldedProb = ThisBlockRightProb[1];
    ThisBlockRightNoRecoProb[1] = ThisBlockRightProb[1];
    noReducedStatesCurrent=Info.RepSize;
    NoSitesToUnfold=0;
    ...
    ImputeChunkMinimac3(group, hapID, endIndex,
              Leftprob[endIndex-startIndex], ThisBlockRightProb[1],
              leftNoRecomProb[endIndex-startIndex],ThisBlockRightProb[1],
    ...
    for (int markerPos=endIndex-1; markerPos>startIndex; markerPos--)
    {
        ...
        if (TargetMarkerPosition!=-1 && tHap->RetrieveMissingScaffoldedHaplotype(hapID, TargetMarkerPosition)=='0')
        {
            RightCondition(markerPos+1,ThisBlockRightProb[1],
        ...
        RightPrecisionJump[markerPos] = RightTranspose(ThisBlockRightProb[0],
                                                  ThisBlockRightNoRecoProb[0],
                                                  Recom[markerPos],
                                                  Info.uniqueCardinality);

        ImputeChunkMinimac3(group, hapID, markerPos,
              Leftprob[markerPos-startIndex], ThisBlockRightProb[0],
              leftNoRecomProb[markerPos-startIndex], ThisBlockRightNoRecoProb[0],
              Leftprob[0],PrevRightFoldedProb);


        ThisBlockRightProb[1]= ThisBlockRightProb[0];
        ThisBlockRightNoRecoProb[1]= ThisBlockRightNoRecoProb[0];
                         ...
                         vector<float> &leftEndProb,vector<float> &rightEndProb)
{
    ...
    ptotal=Pref+Palt;
    (*DosageHap)[position] =  (Palt / ptotal);
...
void MarkovModel::initializeMatricesMinimac3()
{
    ...
    ThisBlockRightNoRecoProb.resize(2);
    ThisBlockRightProb.resize(2);
